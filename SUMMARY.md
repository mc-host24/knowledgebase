# Table of contents
* [Wilkommen](README.md)

## Allgemein
* [Bestellungen und Transaktionen](Allgemein/bestellungen.md)
* [Mein Benutzer](Allgemein/benutzer.md)

## Docker
* [Docker installation](docker/docker-installation.md)
* [Docker Traefik Proxy](docker/docker-traefik-proxy.md)

## Domains
* [Allgemeine Informationen](domains/allgemeine-informationen.md)
* [DNS Einträge erstellen](domains/dns-eintraege.md)
* [Domain Endungen](domains/domain-endungen.md)
* [Domain transferieren](domains/domain-transferieren.md)

## Gameserver

### ARK: Survival Evolved
* [Administration](gameserver/ark/administration.md)
* [Allgemeine Informationen](gameserver/ark/allgemeine-informationen.md)
* [Backup](gameserver/ark/backup.md)
* [Modificationen](gameserver/ark/modificationen.md)

### Landwirtschafts-Simulator 22
* [Mods installieren](gameserver/landwirtschafts-simulator-22/mods-installieren.md)
* [Server einrichten](gameserver/landwirtschafts-simulator-22/server-einrichten.md)
* [Server für Windows konfigurieren](gameserver/landwirtschafts-simulator-22/server-konfigurieren-windows.md)

### Minecraft-Server
* [3D-Weltkarte (Bluemap)](gameserver/minecraft-server/3d-weltkarte.md)
* [Allgemeine Informationen](gameserver/minecraft-server/allgemeine-informationen.md)
* [Bedrock-Server](gameserver/minecraft-server/bedrock-server.md)
* [Bedrock-Spieler](gameserver/minecraft-server/bedrock-spieler.md)
* [BungeeCord installieren](gameserver/minecraft-server/bungeecord.md)
* [Datenpakete installieren](gameserver/minecraft-server/datenpakete-installieren.md)
* [Dynmap installieren](gameserver/minecraft-server/weltkarte.md)
* [Eigene Jar Datei verwenden](gameserver/minecraft-server/eigene-server-jar.md)
* [Eigenes Modpack installieren](gameserver/minecraft-server/modpacks-installieren.md)
* [FastAsyncWorldEdit Installation](gameserver/minecraft-server/FastAsyncWorldEdit-Installation.md)
* [Mehrere Versionen zulassen](gameserver/minecraft-server/mehrere-versionen.md)
* [Mehrere Welten](gameserver/minecraft-server/mehrere-welten.md)
* [Mods auf Fabric installieren](gameserver/minecraft-server/mods-installieren-fabric.md)
* [Mods auf Forge installieren](gameserver/minecraft-server/mods-installieren-forge.md)
* [Mods und Plugins zusammen verwenden](gameserver/minecraft-server/mods-und-plugins.md)
* [Server-Icon benutzen](gameserver/minecraft-server/server-icon.md)
* [Server-Optimierung](gameserver/minecraft-server/optimierung.md)
* [Simple Voice Chat installieren](gameserver/minecraft-server/sprachchat.md)
* [Übliche Spigot Plugins](gameserver/minecraft-server/übliche-spigot-plugins.md)
* [Versionswechsel](gameserver/minecraft-server/version-wechseln.md)

### Terraria
* [Windows-Installation](gameserver/gameserver/terraria/windows-installation.md)

### Hytale Server
* [Hytale Server Performance](gameserver/hytale/performance.md)

## Rootcp
* [Minecraft Version wechseln](rootcp/minecraft-version-wechseln.md)
* [Spigot oder Bungeecord installieren](rootcp/spigot-oder-bungeecord.md)

## Rootserver
* [Fail2ban Installieren](rootserver/fail2ban-installieren.md)
* [FiveM Server auf Ubuntu installieren](rootserver/fivem-server-installation-linux.md)
* [FiveM Server einrichten](rootserver/fivem-server-setup.md)
* [Java 11 installation](rootserver/java-11-setup.md)
* [Java 17 installation](rootserver/java-17-setup.md)
* [Java 8 installation](rootserver/java-8-setup.md)
* [Linux Grundlagen](rootserver/linux-grundlagen.md)
* [Minecraft auto Start](rootserver/minecraft-auto-start.md)
* [Minecraft Server auf Debian 11 installieren](rootserver/minecraft-server-debian-11-installieren.md)
* [Minecraft Server mit unterschiedlichen Java Versionen starten](rootserver/bestimmte-java-version.md)
* [Mit (S)FTP auf Rootserver verbinden](rootserver/sftp-verbinden.md)
* [Moderne PhpMyAdmin Installation](rootserver/modern-phpmyadmin-PHP-8-installation.md)
* [MongoDB installieren](rootserver/mongodb-installieren.md)
* [Nextcloud Installation](rootserver/nextcloud-installation.md)
* [nginx installieren](rootserver/nginx-installieren.md)
* [Nginx Proxy Manager installieren](rootserver/nginx-proxy-manager-installation.md)
* [One-Click Gameserver Installation](rootserver/one-click-gameserver-installer.md)
* [PhpMyAdmin 7.4 Installation](rootserver/phpmyadmin-PHP-7-installation.md)
* [PhpMyAdmin 8.2 Installation](rootserver/phpmyadmin-PHP-8-installation.md)
* [Screen](rootserver/screen.md)
* [SSH Grundlagen](rootserver/ssh-grundlagen.md)
* [SSL Zertifikat Installation](rootserver/ssl-zertifikat-erstellen.md)
* [Sinusbot Installer](rootserver/sinusbot-install.md)
* [Teamspeak³ Server](rootserver/teamspeak-3-server.md)
* [VNC Konsole Server Neustart](rootserver/server-not-aus.md)
* [Virtualhost Apache2](rootserver/virtualhost-apache2.md)
* [Virtualhost Redirection Apache2](rootserver/virtualhost-redirect-apache2.md)

## Webspaces
* [MediaWiki installieren](webspaces/mediawiki-installieren.md)
* [nginx Reverse Proxy](webspaces/nginx-reverse-proxy.md)
* [SSL Zertifikat ausstellen](webspaces/ssl-zertifikat-erstellen.md)
* [WordPress installieren](webspaces/wordpress-installieren.md)

## Sonstiges
* [Discord](https://discord.mc-host24.de)
* [Public API](https://mc-host24.github.io/api-documentation/#/)
* [Teamspeak Server](ts3server://ts.mc-host24.de)